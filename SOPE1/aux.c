...
int hardlinker(Simp_file *files, int size, char *hlinks_txt){
		...
		for(j = i +1; j < size; j++){
			if(strcmp(files[i].name, files[j].name) == 0){
				if(strcmp(files[i].permissions, files[j].permissions) == 0){
					...
					des = open(hlinks_txt, O_WRONLY | O_CREAT | O_APPEND, S_IRUSR | S_IWUSR);
					if(files[i].date > files[j].date){
						//stat_buf1.st_ino = stat_buf2.st_ino;
						unlink(files[i].path);
						link(files[j].path, files[i].path);
						write(des, files[i].path,strlen(files[i].path));
					} else {
						//stat_buf1.st_ino = stat_buf2.st_ino;
						unlink(files[j].path);
						...
						write(des, files[j].path,strlen(files[j].path));
					}
					write(des, "\n", 1);
					close(des);
					//}
...
int write_regular_file(char *path, char *cur_path, char *name, struct stat stat_file){
	...
	pathname = malloc(strlen(cur_path) + strlen(name) + 2);
